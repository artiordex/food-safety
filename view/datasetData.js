...
export const purposes = [
  ...
  {
    id: 'purpose-05',
    title: '위생 단속 적발 예측 및 사전 행정예방 시스템',
    icon: 'ri-shield-user-line',
    description: '지역별 음식점 위생점검 적합률 및 과거 행정처분 이력을 기반으로 단속 리스크가 높은 취약 상권을 파악하고 점주용 자가 진단 가이드를 제시합니다.',
    recommendedDatasets: ['I2560', 'I0470', 'I2500'],
    reason: '단속 적발 이력과 인허가 업소 대장을 결합하여 지자체에는 효율적인 순찰 노선을, 자영업자에게는 준수사항 자가진단 정보를 전파합니다.',
    steps: ['식품위생점검결과(I2560)에서 최근 위반 유형 및 행정처분 연계 데이터 분석', '상권 인허가 대장과 매칭하여 행정동별 위반 빈도수 시각화', '주요 위반 요인(유통기한 경과 등) 자가 위생 진단 푸시 알림 전개'],
    relatedApis: ['식품위생점검결과', '행정처분결과', '인허가업소정보'],
    requiredLevel: '중급 ~ 고급',
    beginnerTip: '추천 데이터세트를 클릭한 뒤, SQL Playground에서 위반내용 컬럼에 `LIKE "%유통기한%"`, `LIKE "%건강진단%"` 등을 검색하여 상위 적발 키워드를 분석해 보세요.',
    devTip: '위반내용 비정형 텍스트에서 명사를 추출하여 자연어(NLP) 가위질을 한 후, 위경도 좌표값과 연계하여 상권별 취약도 히트맵을 생성하는 공간 통계 파이프라인에 최적입니다.'
  },
  {
    id: 'purpose-06',
    title: '수입식품 안전성 및 통관 위해 예측 모델',
    icon: 'ri-ship-2-line',
    description: '수입식품 신고검사 결과 및 부적합 이력을 교차 매칭하여, 통관 검사 시 서류 심사 대상을 자동 분류하고 고위험 수입 업자/제조사를 조기 적발하는 규칙 엔진을 설계합니다.',
    recommendedDatasets: ['C001', 'C003', 'v_imported_food_risk_analysis_dataset'],
    reason: '해외 제조소 및 수입업자의 위해 요소를 등급화하여 정밀 통관이 필요한 제품군을 지능적으로 예보하는 통관 효율화 모델입니다.',
    steps: ['수입식품 신고검사 대장(C003)에서 품목별/제조사별/국가별 부적합률 도출', '수입업자 안전성진단 뷰(v_imported_food_risk_analysis_dataset)와 데이터 결합', '고위험 조합(특정국가+특정제조사+특정품목) 발생 시 실시간 차단 또는 밀착 검사 라우팅'],
    relatedApis: ['수입식품신고검사대장', '수입식품행정처분', '해외위해식품회수정보'],
    requiredLevel: '고급',
    beginnerTip: '수입 신고 시 어떤 식품유형과 국가가 가장 높은 부적합 비율을 기록하는지 로컬 RDBMS 및 라이브 OpenAPI 프리뷰를 번갈아 관찰해 보세요.',
    devTip: '수입업자 등록번호와 제조사 영문명을 고유 키로 결합하고, 특정 제조업체의 누적 부적합 빈도가 3회 이상일 때 즉시 정밀 검사 대상 플래그(FLAG)를 적용하는 실시간 백엔드 규칙 엔진을 구현하기 적합합니다.'
  },
  {
    id: 'purpose-07',
    title: '건강기능식품 영양성분 중복 및 오남용 진단 솔루션',
    icon: 'ri-capsule-line',
    description: '여러 영양제 복용 시 일일 상한 섭취량이나 기능성 성분이 과도하게 중복 섭취되지 않도록 위해도를 실시간으로 연산해주는 지능형 웰니스 서비스입니다.',
    recommendedDatasets: ['I-0050', 'I2819', 'I0030'],
    reason: '다량의 영양제를 소비하는 현대인이 복용 중인 제품명을 검색하면, 개별 성분의 함량을 누적 합산하여 일일 기준표 대비 위해도를 정량 수치로 제공해 안심 섭취를 지원합니다.',
    steps: ['사용자가 복용 중인 여러 영양제의 품목제조보고번호(PRDLST_REPORT_NO) 조회', '영양성분 정보(I2819) 및 개별인정형(I-0050) 함량 컬럼 매핑', '성분별 총합량을 일단위 섭취 상한선 기준치와 자동 대조하여 과다/과소 결과 시각화'],
    relatedApis: ['건강기능식품영양성분', '품목제조신고사항현황', '기능성원료인정현황'],
    requiredLevel: '중급',
    beginnerTip: '비타민이나 유산균 등 실생활과 밀접한 영양성분들의 수치가 테이블 내에 mg, ug 등 어떤 단위 체계로 보관되어 있는지 조회해 보세요.',
    devTip: '제품마다 영양소 함량이 "1정당 함량", "1회 제공량당 함량" 등 비정규화된 텍스트로 저장된 경우가 많아, 정규표현식(RegEx)을 적용해 정량적 mg/g 수치로 환산 정규화하는 데이터 가공 모듈 설계가 핵심입니다.'
  }
];
