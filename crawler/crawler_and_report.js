...
async function main() {
  ...
  try {
    // PK/FK는 STEP 5에서 SQLite 전체 적재 데이터 기준으로 분석한다.
    run('4/9 import_to_sqlite', path.join(DB_DIR, 'import_to_sqlite.js'), '--no-pk-fk');
  } catch (err) {
    logger.error({ err }, 'import_to_sqlite.js 실패 — 파이프라인을 중단합니다.');
    process.exit(1);
  }

  // STEP 5: PK/FK 분석 (analyze_pk_fk.js) - 독립적으로 실행
  try {
