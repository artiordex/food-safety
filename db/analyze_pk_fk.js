...
const KEY_SYNONYM_MAP = new Map();
for (const group of KEY_SYNONYM_GROUPS) {
    const normalizedGroup = group.map(key => String(key).trim().toUpperCase());
    for (const key of normalizedGroup) KEY_SYNONYM_MAP.set(key, normalizedGroup);
}

function getKeySynonyms(fieldName) {
    return KEY_SYNONYM_MAP.get(String(fieldName || '').trim().toUpperCase()) || [];
}
...
function findSimilarPkFields(upperField, pkFieldIndex) {
    ...
    const synonymMatches = getKeySynonyms(upperField)
        .filter(key => key !== upperField && pkFieldIndex.has(key))
        .map(key => ({ key, similarity: 1.0, exact: false, synonym: true }));
    if (synonymMatches.length > 0) return synonymMatches.slice(0, 3);
...
// 주소/GIS 세부 구성요소는 식별자가 아니라 위치 속성이므로 단독 PK 후보에서 제외한다.
const LOCATION_COMPONENT_FIELD_PATTERNS = [
    /^SPCPPISSNO$/i,
    /^SPCPPDONG$/i,
    /^SPCLADDR$/i,
    /^ROADNMADDR/i,
...
// 첨부/이미지/용어 필드는 고유값처럼 보여도 관계 식별자가 아니므로 단독 PK 후보에서 제외한다.
const DESCRIPTIVE_OR_FILE_FIELD_PATTERNS = [
    /^ATT_FILE_NO(_|$)/i,
    /^FILE_PATH$/i,
    /^IMAGE_PATH$/i,
    /^IMG(_|$)/i,
...
const WEAK_FIELD_PATTERNS = [
    ...
    /NM$/i, /NAME$/i, /DCNM$/i,
    /ADDR$/i, /ADDRESS$/i,
    /^FRMLCUNIT$/i, /^FRMLC_UNIT$/i,
    /^SPEC_VAL_SUMUP$/i, /^SPECVALSUMUP$/i, /_SUMUP$/i,
    /^SORC$/i, /^SOURCE$/i,
    /ZIPNO$/i, /ZIP_NO$/i,
    /DT$/i, /_YMD$/i, /DTM$/i, /DATE$/i,
...
const AGGREGATE_FIELD_PATTERNS = [
    ...
    /^PB$/i,                         // 납 측정값
    /^CDMM$/i, /^COPPR$/i, /^AS_$/i, /^MRC$/i, /^CR6$/i,
    /^ZINC$/i, /^NICKEL$/i, /^IMCOW$/i, /^ORGNICPH$/i,
    /^PCT$/i, /^CYN$/i, /^PHNL$/i, /^BENZ$/i, /^TOLUE$/i,
    /^ETBEN$/i, /^XYLEN$/i, /^TPH$/i, /^TCE$/i, /^PCE$/i,
    /^HYDRIONDNSTY$/i,
...
function isOrderField(fieldName) {
    return matchesAny(String(fieldName || ''), ORDER_FIELD_PATTERNS);
}
...
function analyzePkCandidatesForTable(ds, records, fieldEntropyMap = null) {
    ...
    for (const field of validFields) {
        if (isBadPkField(field)) continue;
    ...
    if (records.length > 0 && compositeBase.length >= 2) {
        ...
        for (const combo of makeCompositeKeyCombos(compositeBase)) {
            const hasOnlyOrderFields = combo.every(isOrderField);
            const hasRealIdentifier = combo.some(f => !isOrderField(f));
            if (hasOnlyOrderFields || !hasRealIdentifier) continue;
            ...
            if (combo.some(f => !isOrderField(f) && /SEQ|_NO|NO$/i.test(f))) score += 10;
            if (combo.some(isOrderField)) score -= 5;
...
function formatPkFieldsForMd(pk) {
    const fields = Array.isArray(pk?.fields) ? pk.fields.filter(Boolean) : [];
    return fields.length > 0 ? fields.join(' + ') : '-';
}

const LOGICAL_NAME_OVERRIDES = {
    FOOD_CD: '식품코드',
    FOOD_OR_CD: '식품출처코드',
    FOOD_CAT1_CD: '식품대분류코드',
    FOOD_CAT2_CD: '식품중분류코드',
    FOOD_CAT3_CD: '식품소분류코드',
    FOOD_CAT4_CD: '식품세부분류코드',
    FOOD_REF_CD: '대표식품코드',
    DB_GRP_CM: 'DB그룹코드',
    DB_CLASS_CM: 'DB분류코드',
...
function resolveLogicalName(fieldName, logicalName) {
    const field = String(fieldName || '').trim().toUpperCase();
    const name = String(logicalName || '').trim();
    if (LOGICAL_NAME_OVERRIDES[field]) return LOGICAL_NAME_OVERRIDES[field];
    if (name && name.toUpperCase() !== field && hasKoreanText(name)) return name;
    return name || fieldName || '-';
}

function formatPkLogicalNamesForMd(pk) {
    const fields = Array.isArray(pk?.fields) ? pk.fields : [];
    const korNames = Array.isArray(pk?.kor_names) ? pk.kor_names : [];
    const names = fields.map((field, idx) => {
        return resolveLogicalName(field, korNames[idx]);
    }).filter(Boolean);
...
async function run(options) {
    ...
    log('STEP', 'OPERTN_CITYPOINT', 'crawl_cache.json 로드');
    ...
    log('STEP', 'OPERTN_CITYPOINT', '샘플 및 SQLite 적재 데이터 로드');
    ...
    log('STEP', 'OPERTN_CITYPOINT', 'PK/FK 후보 분석');
    ...
    log('STEP', 'OPERTN_CITYPOINT', '결과 파일 생성');
