# 데이터셋 사용 시나리오 분석 결과
> 생성일시: 2026-06-19T07:08:26.015+09:00

- 전체 시나리오: **31개** (Star: 20개, Chain: 11개)
- 전체 관계: **116개**
- HIGH 신뢰도 시나리오: **4개**
- SQL 검증 오류 시나리오: **0개**
- 통합 테마 후보: **6개**
- 통합 연결 경로: **30개**
- Louvain 데이터 세트 후보: **15개** (modularity 0.8284)
...
|---|---|---:|---:|---|---|---|
| LOUVAIN_01 | 시험검사 기준규격 연계 데이터 세트 | 9 | 28 | 기준규격정보, testitm_cd, kor_nm, use_yn, pc_kor_nm, t_kor_nm | TESTITM_CD(17), PRDLST_CD(7), VALDMANLI(1), CHOICIMPROPT(1), CHOICFIT(1), CMMNSPECCD(1) | 기준규격정보(7), 코드정보(2) |
| LOUVAIN_11 | 건강기능식품 GMP 품목 연계 데이터 세트 | 7 | 20 | 건강기능식품, gmp, 현황, induty_cd_nm, prdlst_nm, pog_daycnt | LCNS_NO(19), PRDLST_REPORT_NO(1) | 건강기능식품(6), 업체인허가현황(1) |
| LOUVAIN_04 | HACCP 및 업체 인허가 연계 데이터 세트 | 12 | 19 | 축산물, haccp, prdlst_nm, induty_cd_nm, prdlst_dcnm, 식품 | LCNS_NO(18), RAWMTRLORDNO(1) | 축산물(5), 식품 등(3), 코드정보(1), 어린이식품안전관리(1), HACCP지정현황(1), 업체인허가현황(1) |
| LOUVAIN_07 | 식품위해 행정처분 연계 데이터 세트 | 7 | 16 | 식품위해관리, 행정처분결과, prcscitypoint_bsshnm, induty_cd_nm, dsps_dcsndt, dsps_bgndt | LCNS_NO(7), BRCD_NO(3), DSPSDTLSSEQ(3), RTRVLDSUSESEQ(2), PRDLST_REPORT_NO(1) | 식품위해관리(7) |
| LOUVAIN_12 | 위생용품 인허가 연계 데이터 세트 | 5 | 6 | 위생용품, prdlst_nm, induty_nm, locp_addr, instt_nm, prdlst_dcnm | LCNS_NO(6) | 위생용품(4), 폐업정보(1) |
| LOUVAIN_10 | HACCP 및 업체 인허가 연계 데이터 세트 | 4 | 3 | haccp, 식품, site_addr, 현황, induty_cd_nm, prdlst_nm | LCNS_NO(3) | 식품 등(3), HACCP지정현황(1) |
...
| LOUVAIN_09 | 이력추적관리 연계 데이터 세트 | 3 | 3 | 이력추적관리, 수산물이력정보, hist_trace_reg_no, prdlst_group_dvs_nm, goods_nm, lotno_wrhousng | HISTTRACEREGNO(3) | 이력추적관리(3) |
| LOUVAIN_14 | 식품 등 연계 데이터 세트 | 3 | 3 | 식품, 현황, evl_dt, 식품위생등급평가관리내역, 주류제조.면허자, 식품제조.가공영업 | LCNS_NO(3) | 식품 등(3) |
| LOUVAIN_06 | 시험검사 기준규격 연계 데이터 세트 | 3 | 2 | 검사기관정보, 현황, 지정, appn_bgn_dt, prsec_instt_rcogn_no, appn_end_dt | WORKSCOPE(1), PRSECINSTTRCOGNNO(1) | 검사기관정보(3) |
| LOUVAIN_03 | 시험검사 기준규격 연계 데이터 세트 | 2 | 1 | 현황, lmo, 기준규격정보, 식품원료의, 한시적, 기준 | PRPOS(1) | 기준규격정보(1), 수입식품 등(1) |
| LOUVAIN_05 | 수질환경 정보 연계 데이터 세트 | 2 | 1 | 수질환경정보, 지하수수질측정망, 측정결과, 토양지하수, 토양실태조사정보, cdmm | AS(1) | 수질환경정보(2) |
| LOUVAIN_08 | 이력추적관리 연계 데이터 세트 | 2 | 1 | 이력추적관리, 쇠고기, 국내, 이력추적, 생산정보, 정보 | VACINLASTINOCLOPNO(1) | 이력추적관리(2) |
| LOUVAIN_13 | 수입식품 업체 연계 데이터 세트 | 2 | 1 | 수입식품, 식품등수입판매업정보, 수입식품등영업신고대장, induty_nm, locp_addr, instt_nm | LCNS_NO(1) | 수입식품 등(2) |
...
|---|---|---:|---:|---|---|
| THEME_01 | 행정처분결과·인허가 데이터 세트 | 34 | 64 | I1220 식품제조가공업정보 / I2530 시험항목코드 | LCNS_NO(40), PRDLST_CD(7), PRDLST_REPORT_NO(7), TESTITM_CD(6), CMMN_SPEC_CD(2) |
| THEME_02 | 위생용품품목제조보고·위생용품영업정보 데이터 세트 | 4 | 5 | I2713 위생용품영업정보 / I2711 위생용품품목제조보고 | LCNS_NO(3), PRDLST_REPORT_NO(2) |
| THEME_03 | 건강기능식품·생산실적 데이터 세트 | 3 | 2 | I0310 건강기능식품 생산실적 보고 품목 현황 / I0030 건강기능식품 품목제조 신고사항 현황 | PRDLST_REPORT_NO(2) |
| THEME_04 | 식품접객업정보·식품접객업소 데이터 세트 | 3 | 2 | I1200 식품접객업정보 / C004 식품접객업소 위생등급 지정현황 | LCNS_NO(2) |
| THEME_05 | 과태료부과기준·처분기준코드 데이터 세트 | 2 | 1 | I1670 과태료부과기준 / I2550 처분기준코드 | DSPS_STDR_CD(1) |
| THEME_06 | 식품등수입판매업정보·수입식품등영업신고대장 데이터 세트 | 2 | 1 | I1260 식품등수입판매업정보 / C001 수입식품등영업신고대장 | LCNS_NO(1) |

## PK/FK 그래프 기반 대표 연결 경로

| From | To | 홉 수 | 경로 |
|---|---|---:|---|
| C002 식품(첨가물)품목제조보고(원재료) | I1220 식품제조가공업정보 | 1 | C002 -> I1220 |
| C002 식품(첨가물)품목제조보고(원재료) | I1250 식품(첨가물)품목제조보고 | 1 | C002 -> I1250 |
...
| I2620 검사부적합(국내) | I1300 축산물 가공업허가정보 | 1 | I2620 -> I1300 |
| I2620 검사부적합(국내) | I0490 회수.판매중지 정보 | 1 | I2620 -> I0490 |

## SCN_002 — `VALDMANLI` 기반 (HIGH, 100점)

**참여 데이터셋** (2개): I2580, I2600

| From | 컬럼명 | To | 컬럼명 | JOIN 유형 | 카디널리티 | 매칭 | A비율 | B비율 | 신뢰도 | 샘플값 |
|---|---|---|---|---|---|---|---|---|---|---|
| 개별기준규격 | 유효자리(VALD_MANLI) | 공통기준규격 | 유효자리(VALD_MANLI) | INNER JOIN | N:M | 4 | 100% | 100% | HIGH | `1, 2` |
...
## SCN_003 — `CHOICIMPROPT` 기반 (HIGH, 100점)

**참여 데이터셋** (2개): I2580, I2600

| From | 컬럼명 | To | 컬럼명 | JOIN 유형 | 카디널리티 | 매칭 | A비율 | B비율 | 신뢰도 | 샘플값 |
|---|---|---|---|---|---|---|---|---|---|---|
| 개별기준규격 | 선택형부적합코드(CHOIC_IMPROPT) | 공통기준규격 | 선택형부적합코드(CHOIC_IMPROPT) | INNER JOIN | N:M | 3 | 100% | 100% | HIGH | `02, 04` |
...
FROM "I2580" A
INNER JOIN "I2600" B
  ON A."CHOIC_IMPROPT" = B."CHOIC_IMPROPT"
WHERE A."CHOIC_IMPROPT" IS NOT NULL AND A."CHOIC_IMPROPT" != ''
LIMIT 100;
```

## SCN_004 — `CHOICFIT` 기반 (HIGH, 100점)

**참여 데이터셋** (2개): I2580, I2600

| From | 컬럼명 | To | 컬럼명 | JOIN 유형 | 카디널리티 | 매칭 | A비율 | B비율 | 신뢰도 | 샘플값 |
|---|---|---|---|---|---|---|---|---|---|---|
| 개별기준규격 | 선택형적합코드(CHOIC_FIT) | 공통기준규격 | 선택형적합코드(CHOIC_FIT) | INNER JOIN | N:M | 3 | 100% | 100% | HIGH | `01, 03` |
...
FROM "I2580" A
INNER JOIN "I2600" B
  ON A."CHOIC_FIT" = B."CHOIC_FIT"
WHERE A."CHOIC_FIT" IS NOT NULL AND A."CHOIC_FIT" != ''
LIMIT 100;
```

## SCN_005 — `VACINLASTINOCLOPNO` 기반 (HIGH, 93점)

**참여 데이터셋** (2개): I1810, I1820

| From | 컬럼명 | To | 컬럼명 | JOIN 유형 | 카디널리티 | 매칭 | A비율 | B비율 | 신뢰도 | 샘플값 |
|---|---|---|---|---|---|---|---|---|---|---|
| 쇠고기(국내)이력추적 생산정보 | 백신최종접종차수(VACIN_LAST_INOCL_OPNO) | 쇠고기(국내)이력추적 정보 | 백신최종접종차수(VACIN_LAST_INOCL_OPNO) | INNER JOIN | N:M | 26 | 93% | 93% | HIGH | `8, 9` |
...
## SCN_CHAIN_002 — `CHAIN:VALDMANLI+TESTITM_CD+PRDLST_CD` 기반 (MEDIUM, 69점) 🔗 브릿지: `I2580`

**참여 데이터셋** (4개): I2580, I2600, I0950, I0940

| From | 컬럼명 | To | 컬럼명 | JOIN 유형 | 카디널리티 | 매칭 | A비율 | B비율 | 신뢰도 | 샘플값 |
|---|---|---|---|---|---|---|---|---|---|---|
| 개별기준규격 | 유효자리(VALD_MANLI) | 공통기준규격 | 유효자리(VALD_MANLI) | INNER JOIN | N:M | 4 | 100% | 100% | HIGH | `1, 2` |
| 식품첨가물공전 | 시험항목코드(TESTITM_CD) | 개별기준규격 | 시험항목코드(TESTITM_CD) | LEFT JOIN (A→B) | N:M | 32 | 94% | 15% | MEDIUM | `A40005, A10008` |
...
## SCN_CHAIN_001 — `CHAIN:VALDMANLI+CMMNSPECCD+PRDLST_CD` 기반 (MEDIUM, 68점) 🔗 브릿지: `I2600`

**참여 데이터셋** (4개): I2600, I2580, I2590, I2610
...
SELECT
  ...
  C."HRNK_CMMN_SPEC_CD" AS "C_상위공통기준규격코드",
  C."LV" AS "C_레벨",
  C."DFN" AS "C_정의",
  C."USE_YN" AS "C_사용여부",
...
INNER JOIN "I2580" B
  ON A."VALD_MANLI" = B."VALD_MANLI"
LEFT JOIN "I2590" C
  ON A."CMMN_SPEC_CD" = C."CMMN_SPEC_CD"
LEFT JOIN "I2610" D
  ON A."PRDLST_CD" = D."PRDLST_CD"
WHERE A."VALD_MANLI" IS NOT NULL AND A."VALD_MANLI" != ''
LIMIT 100;
...
## SCN_007 — `CMMNSPECCD` 기반 (MEDIUM, 64점)
...
## SCN_CHAIN_004 — `CHAIN:RTRVLDSUSESEQ+PRDLST_REPORT_NO` 기반 (MEDIUM, 54점) 🔗 브릿지: `I2620`

**참여 데이터셋** (3개): I2620, I2640, I0490

| From | 컬럼명 | To | 컬럼명 | JOIN 유형 | 카디널리티 | 매칭 | A비율 | B비율 | 신뢰도 | 샘플값 |
|---|---|---|---|---|---|---|---|---|---|---|
| 검사부적합(국내) | 회수폐기일련번호(RTRVLDSUSE_SEQ) | 검사부적합 현황(농산물) | 회수폐기일련번호(RTRVLDSUSE_SEQ) | INNER JOIN | N:M | 232 | 68% | 100% | HIGH | `3000216762, 3000216875` |
| 회수.판매중지 정보 | 품목제조보고번호(PRDLST_REPORT_NO) | 검사부적합(국내) | 품목제조보고번호(PRDLST_REPORT_NO) | LEFT JOIN (B→A) | 1:N | 18 | 12% | 35% | LOW | `2017047206438, 2018004236216` |
...
## SCN_011 — `WORKSCOPE` 기반 (MEDIUM, 50점)

**참여 데이터셋** (2개): I0890, I0900

| From | 컬럼명 | To | 컬럼명 | JOIN 유형 | 카디널리티 | 매칭 | A비율 | B비율 | 신뢰도 | 샘플값 |
|---|---|---|---|---|---|---|---|---|---|---|
| 식품위생검사기관 지정 현황 | 업무범위(WORK_SCOPE) | 축산물위생검사기관 지정 현황 | 업무범위(WORK_SCOPE) | INNER JOIN | N:M | 2 | 50% | 50% | MEDIUM | `자가품질위탁검사, 자가품질위탁검사 수입검사` |
...
SELECT
  ...
  B."BSSH_NM" AS "B_검사기관명",
  B."PRSDNT_NM" AS "B_대표자",
  B."ADDR" AS "B_주소",
  B."APPN_BGN_DT" AS "B_지정시작일자",
  B."APPN_END_DT" AS "B_지정종료일자",
  B."WORK_SCOPE" AS "B_업무범위"
FROM "I0890" A
INNER JOIN "I0900" B
...
## SCN_006 — `RTRVLDSUSESEQ` 기반 (MEDIUM, 44점)
...
|---|---|---|---|---|---|---|---|---|---|---|
| 검사부적합(국내) | 회수폐기일련번호(RTRVLDSUSE_SEQ) | 검사부적합 현황(농산물) | 회수폐기일련번호(RTRVLDSUSE_SEQ) | INNER JOIN | N:M | 232 | 68% | 100% | HIGH | `3000216762, 3000216875` |
| 회수.판매중지 정보 | 회수.판매중지 일련번호(RTRVLDSUSE_SEQ) | 검사부적합 현황(농산물) | 회수폐기일련번호(RTRVLDSUSE_SEQ) | LEFT JOIN (B→A) | 1:N | 12 | 3% | 5% | LOW | `3000226584, 3000225170` |
...
INNER JOIN "I2640" B
  ON A."RTRVLDSUSE_SEQ" = B."RTRVLDSUSE_SEQ"
LEFT JOIN "I0490" C
  ON B."RTRVLDSUSE_SEQ" = C."RTRVLDSUSE_SEQ"
WHERE A."RTRVLDSUSE_SEQ" IS NOT NULL AND A."RTRVLDSUSE_SEQ" != ''
LIMIT 100;
```

## SCN_CHAIN_010 — `CHAIN:TESTITM_CD+PRDLST_CD` 기반 (MEDIUM, 42점) 🔗 브릿지: `I2610`
...
## SCN_CHAIN_011 — `CHAIN:PRDLST_CD+TESTITM_CD` 기반 (MEDIUM, 42점) 🔗 브릿지: `I0960`

**참여 데이터셋** (3개): I0960, I2580, I2530

| From | 컬럼명 | To | 컬럼명 | JOIN 유형 | 카디널리티 | 매칭 | A비율 | B비율 | 신뢰도 | 샘플값 |
|---|---|---|---|---|---|---|---|---|---|---|
| 건강기능식품공전 | 품목코드(PRDLST_CD) | 개별기준규격 | 품목분류코드(PRDLST_CD) | LEFT JOIN (B→A) | N:M | 112 | 34% | 61% | MEDIUM | `E0301700000000, E0317300000000` |
| 건강기능식품공전 | 시험항목코드(TESTITM_CD) | 시험항목코드 | 시험항목코드(TESTITM_CD) | LEFT JOIN (A→B) | 1:N | 103 | 61% | 10% | LOW | `A40128, A40155` |
...
## SCN_008 — `BRCD_NO` 기반 (MEDIUM, 40점)

**참여 데이터셋** (3개): I2620, I2640, I0490

| From | 컬럼명 | To | 컬럼명 | JOIN 유형 | 카디널리티 | 매칭 | A비율 | B비율 | 신뢰도 | 샘플값 |
|---|---|---|---|---|---|---|---|---|---|---|
| 검사부적합(국내) | 바코드번호(BRCDNO) | 검사부적합 현황(농산물) | 바코드번호(BRCDNO) | LEFT JOIN (B→A) | N:M | 16 | 27% | 100% | MEDIUM | `8809225440811, 8809153040398` |
| 회수.판매중지 정보 | 바코드번호(BRCDNO) | 검사부적합(국내) | 바코드번호(BRCDNO) | LEFT JOIN (B→A) | N:M | 26 | 16% | 44% | LOW | `8809045620752, 8809458913335` |
| 회수.판매중지 정보 | 바코드번호(BRCDNO) | 검사부적합 현황(농산물) | 바코드번호(BRCDNO) | LEFT JOIN (B→A) | N:M | 8 | 5% | 50% | LOW | `8809052505004, 1100001247554` |
...
## SCN_001 — `LCNS_NO` 기반 (LOW, 30점)

**참여 데이터셋** (42개): C001, I1260, I2500, I2560, I0680, I1540, I-0020, I0630, I2860, I1220, I2859, I0030, I1200, I2861, I2620, I2640, C003, I1300, I1310, I1250, I0060, I0310, I2711, I2713, I2712, I0490, C002, I1230, I2851, I2823, I0470, I0480, I0481, I0080, I0610, C006, I1320, I2852, I0482, I0580, I0300, I1420

| From | 컬럼명 | To | 컬럼명 | JOIN 유형 | 카디널리티 | 매칭 | A비율 | B비율 | 신뢰도 | 샘플값 |
|---|---|---|---|---|---|---|---|---|---|---|
| 수입식품등영업신고대장 | 인허가번호(LCNS_NO) | 식품등수입판매업정보 | 인허가 번호(LCNS_NO) | INNER JOIN | 1:1 | 1000 | 100% | 100% | HIGH | `20200009171, 20220020542` |
| 인허가 업소 정보 | 영업고유구분번호(인허가번호)(LCNS_NO) | 영업소재지 GIS 코드 | 인허가번호(LCNS_NO) | INNER JOIN | 1:1 | 998 | 100% | 100% | HIGH | `18820308001, 18830478001` |
| 위생관리등급별 업소 현황 | 인허가번호(LCNS_NO) | 식품위생등급평가관리내역 | 인허가번호(LCNS_NO) | INNER JOIN | N:M | 758 | 100% | 100% | HIGH | `20250019495, 20250016500` |
| 건강기능식품 전문.벤처제조업인허가 현황 | 인허가 번호(LCNS_NO) | 건강기능식품GMP 지정 현황 | 업고고유번호(LCNS_NO) | INNER JOIN | 1:1 | 476 | 86% | 79% | HIGH | `20140020025, 20230001042` |
...
| 검사부적합(국내) | 업체인허가번호(LCNS_NO) | 검사부적합 현황(농산물) | 업체인허가번호(LCNS_NO) | LEFT JOIN (B→A) | N:M | 12 | 14% | 100% | MEDIUM | `20250017312, 20030115203` |
...
| 주류제조.면허자 식품제조.가공영업 등록 현황 | 인허가번호(LCNS_NO) | 위생관리등급별 업소 현황 | 인허가번호(LCNS_NO) | LEFT JOIN (B→A) | 1:N | 373 | 37% | 49% | MEDIUM | `20190012114, 20172860142` |
| 주류제조.면허자 식품제조.가공영업 등록 현황 | 인허가번호(LCNS_NO) | 식품위생등급평가관리내역 | 인허가번호(LCNS_NO) | LEFT JOIN (B→A) | 1:N | 373 | 37% | 49% | MEDIUM | `20190012114, 20172860142` |
...
| 위생용품품목제조보고(원재료) | 인허가번호(LCNS_NO) | 위생용품영업정보 | 인허가번호(LCNS_NO) | LEFT JOIN (A→B) | 1:N | 114 | 71% | 11% | MEDIUM | `20259370002, 20259370003` |
| 회수.판매중지 정보 | 업체인허가번호(LCNS_NO) | 검사부적합(국내) | 업체인허가번호(LCNS_NO) | LEFT JOIN (B→A) | N:M | 35 | 16% | 41% | LOW | `20200574235, 20120254022` |
| 건강기능식품 품목제조신고(원재료) | 인허가번호(LCNS_NO) | 건강기능식품 품목제조 신고사항 현황 | 인허가번호(LCNS_NO) | LEFT JOIN (A→B) | N:M | 14 | 47% | 10% | LOW | `20040015039, 20040015070` |
| 회수.판매중지 정보 | 업체인허가번호(LCNS_NO) | 검사부적합 현황(농산물) | 업체인허가번호(LCNS_NO) | LEFT JOIN (B→A) | N:M | 6 | 3% | 50% | LOW | `20160333083, 20000320226` |
...
| 행정처분결과 | 인허가번호(LCNS_NO) | 행정처분결과(식품제조가공업) | 인허가번호(LCNS_NO) | LEFT JOIN (B→A) | 1:1 | 49 | 5% | 19% | LOW | `20220803181, 20170202581` |
| 행정처분결과 | 인허가번호(LCNS_NO) | 행정처분결과(식품판매업) | 인허가번호(LCNS_NO) | LEFT JOIN (B→A) | 1:1 | 80 | 8% | 12% | LOW | `20150386322, 20190073611` |
...
| 축산물HACCP 지정정보 | 인허가번호(LCNS_NO) | 축산물 가공업허가정보 | 인허가번호(LCNS_NO) | LEFT JOIN (A→B) | 1:1 | 92 | 10% | 9% | LOW | `20230904005, 20250371008` |
...
| 건강기능식품 품목제조 신고사항 현황 | 인허가번호(LCNS_NO) | 생산중단제품정보 | 인허가번호(LCNS_NO) | LEFT JOIN (B→A) | N:M | 8 | 6% | 6% | LOW | `20040020002, 20100019004` |
...
| 축산물HACCP 지정정보 | 인허가번호(LCNS_NO) | 축산물 식육포장처리업영업허가대장 | 인허가 번호(LCNS_NO) | LEFT JOIN (A→B) | 1:1 | 49 | 5% | 5% | LOW | `20240054282, 20260333162` |
| 축산물품목제조보고(원재료) | 인허가번호(LCNS_NO) | 축산물 가공업허가정보 | 인허가번호(LCNS_NO) | LEFT JOIN (A→B) | 1:N | 11 | 10% | 1% | LOW | `20230881186, 20180455015` |
| 행정처분결과(수입식품업) | 인허가번호(LCNS_NO) | 검사부적합 현황(농산물) | 업체인허가번호(LCNS_NO) | LEFT JOIN (B→A) | 1:N | 1 | 2% | 8% | LOW | `20250017312` |
| 축산물HACCP 지정정보 | 인허가번호(LCNS_NO) | 축산물 품목제조정보 | 인허가번호(LCNS_NO) | LEFT JOIN (B→A) | 1:N | 1 | 0% | 10% | LOW | `19790532001` |
| HACCP 적용업소 지정 현황 | 인허가번호(LCNS_NO) | 식품제조가공업정보 | 인허가 번호(LCNS_NO) | LEFT JOIN (A→B) | 1:N | 24 | 5% | 2% | LOW | `20040529135, 19940529058` |
| 식품(첨가물)품목제조보고(원재료) | 인허가번호(LCNS_NO) | 식품제조가공업정보 | 인허가 번호(LCNS_NO) | LEFT JOIN (A→B) | 1:N | 17 | 6% | 2% | LOW | `19950433026, 20080449004` |
| 건강기능식품 전문.벤처제조업인허가 현황 | 인허가 번호(LCNS_NO) | 생산중단제품정보 | 인허가번호(LCNS_NO) | LEFT JOIN (B→A) | 1:N | 8 | 1% | 6% | LOW | `20100019004, 20190004553` |
| 건강기능식품GMP 지정 현황 | 업고고유번호(LCNS_NO) | 생산중단제품정보 | 인허가번호(LCNS_NO) | LEFT JOIN (B→A) | 1:N | 8 | 1% | 6% | LOW | `20190004553, 20100019004` |
| 생산중단제품정보 | 인허가번호(LCNS_NO) | 건강기능식품업소 인허가 변경 정보 | 인허가번호(LCNS_NO) | LEFT JOIN (A→B) | N:M | 7 | 5% | 2% | LOW | `20100019004, 20040020002` |
...
## SCN_CHAIN_005 — `CHAIN:WORKSCOPE+PRSECINSTTRCOGNNO` 기반 (LOW, 30점) 🔗 브릿지: `I0890`

**참여 데이터셋** (3개): I0890, I0900, I0910
...
SELECT
  ...
  B."APPN_END_DT" AS "B_지정종료일자",
  B."WORK_SCOPE" AS "B_업무범위",
  C."PRSEC_INSTT_RCOGN_NO" AS "C_인정번호",
  C."BSSH_NM" AS "C_기관명",
  C."PRSDNT_NM" AS "C_대표자",
  C."APPN_BGN_DT" AS "C_지정일자",
  C."PRSEC_ITM_CD_NM" AS "C_검사항목",
  C."TELNO" AS "C_전화번호",
  C."BSSH_ADDR" AS "C_주소"
FROM "I0890" A
INNER JOIN "I0900" B
  ON A."WORK_SCOPE" = B."WORK_SCOPE"
LEFT JOIN "I0910" C
  ON A."PRSEC_INSTT_RCOGN_NO" = C."PRSEC_INSTT_RCOGN_NO"
WHERE A."WORK_SCOPE" IS NOT NULL AND A."WORK_SCOPE" != ''
LIMIT 100;
```

## SCN_CHAIN_006 — `CHAIN:LCNS_NO+PRDLST_REPORT_NO` 기반 (LOW, 30점) 🔗 브릿지: `C003`
...
## SCN_CHAIN_007 — `CHAIN:LCNS_NO+PRDLST_REPORT_NO` 기반 (LOW, 25점) 🔗 브릿지: `I0310`
...
## SCN_010 — `TESTITM_CD` 기반 (LOW, 24점)

**참여 데이터셋** (7개): I0950, I2580, I0940, I2610, I2600, I0960, I2530
...
| 건강기능식품공전 | 시험항목코드(TESTITM_CD) | 개별기준규격 | 시험항목코드(TESTITM_CD) | LEFT JOIN (A→B) | N:M | 59 | 35% | 27% | LOW | `B90438, B90435` |
| 시험항목코드 | 시험항목코드(TESTITM_CD) | 공통기준제외 | 시험항목코드(TESTITM_CD) | LEFT JOIN (B→A) | 1:N | 4 | 0% | 57% | LOW | `B10002, B10004` |
| 식품첨가물공전 | 시험항목코드(TESTITM_CD) | 시험항목코드 | 시험항목코드(TESTITM_CD) | LEFT JOIN (A→B) | 1:N | 17 | 50% | 2% | LOW | `A40005, A10008` |
| 식품첨가물공전 | 시험항목코드(TESTITM_CD) | 공통기준제외 | 시험항목코드(TESTITM_CD) | LEFT JOIN (B→A) | N:M | 2 | 6% | 29% | LOW | `B10004, B20016` |
| 식품첨가물공전 | 시험항목코드(TESTITM_CD) | 건강기능식품공전 | 시험항목코드(TESTITM_CD) | LEFT JOIN (A→B) | N:M | 6 | 18% | 4% | LOW | `A40005, B10001` |
...
| 식품첨가물공전 | 시험항목코드(TESTITM_CD) | 공통기준규격 | 시험항목코드(TESTITM_CD) | LEFT JOIN (A→B) | N:M | 4 | 12% | 1% | LOW | `B10001, B90054` |
| 식품용 기구 및 용기.포장 공전 | 시험항목코드(TESTITM_CD) | 식품첨가물공전 | 시험항목코드(TESTITM_CD) | LEFT JOIN (B→A) | N:M | 3 | 4% | 9% | LOW | `B10001, B10003` |
| 식품용 기구 및 용기.포장 공전 | 시험항목코드(TESTITM_CD) | 건강기능식품공전 | 시험항목코드(TESTITM_CD) | LEFT JOIN (A→B) | N:M | 4 | 6% | 2% | LOW | `B10001, B10003` |
...
SELECT
  ...
  D."TESTITM_CD" AS "D_시험항목코드",
  D."LAST_UPDT_DTM" AS "D_최종수정일시",
  E."CMMN_SPEC_SEQ" AS "E_공통기준종류코드일련번호",
  E."CMMN_SPEC_CD" AS "E_공통기준종류코드",
  E."SPEC_NM" AS "E_공통기준종류명",
  E."PRDLST_CD" AS "E_품목분류코드",
  E."PRDLST_CD_NM" AS "E_품목명",
  E."TESTITM_CD" AS "E_시험항목코드",
  E."TESTITM_NM" AS "E_시험항목명",
  E."FNPRT_ITM_NM" AS "E_세부항목명",
  E."ATTRB_SEQ" AS "E_단서조항일련번호",
  E."PIAM_KOR_NM" AS "E_단서조항명",
  E."SPEC_VAL" AS "E_기준규격",
  E."SPEC_VAL_SUMUP" AS "E_기준규격요약",
  E."VALD_BEGN_DT" AS "E_유효개시일",
  E."VALD_END_DT" AS "E_유효종료일",
  E."SORC" AS "E_출처",
  E."VALD_MANLI" AS "E_유효자리",
  E."JDGMNT_FOM_CD" AS "E_판정형식코드",
  E."A079_FNPRT_CD_NM" AS "E_판정형식명",
  E."MXMM_VAL" AS "E_최대값",
  E."MXMM_VAL_DVS_CD" AS "E_최대값구분코드",
  E."A081_FNPRT_CD_NM" AS "E_최대값구분명",
  E."MIMM_VAL" AS "E_최소값",
  E."MIMM_VAL_DVS_CD" AS "E_최소값구분코드",
  E."A080_FNPRT_CD_NM" AS "E_최소값구분명",
  E."CHOIC_FIT" AS "E_선택형적합코드",
  E."A082_CF_FNPRT_CD_NM" AS "E_선택형적합명",
  E."CHOIC_IMPROPT" AS "E_선택형부적합코드",
  E."A082_CI_FNPRT_CD_NM" AS "E_선택형부적합명",
  E."MCRRGNSM_2N" AS "E_미생물2N",
  E."MCRRGNSM_2C" AS "E_미생물2C",
  E."MCRRGNSM_2M" AS "E_미생물2M",
  E."MCRRGNSM_3M" AS "E_미생물3M",
  E."FNPRT_ITM_INCLS_YN" AS "E_세부항목포함여부",
  E."INJRY_YN" AS "E_위해여부",
  E."EMPHS_PRSEC_TESTITM_YN" AS "E_중점검사시험항목여부",
  E."MONTRNG_TESTITM_YN" AS "E_감시시험항목여부",
  E."RVLV_ELSE_TESTITM_YN" AS "E_공전외시험항목여부",
  E."NTR_PRSEC_ITM_YN" AS "E_자품검사시험항목여부",
  E."UNIT_CD" AS "E_단위코드",
  E."UNIT_NM" AS "E_단위명",
  E."UPDT_PRVNS" AS "E_수정사유",
  E."LAST_UPDT_DTM" AS "E_최종수정일시"
FROM "I0950" A
LEFT JOIN "I2580" B
  ON A."TESTITM_CD" = B."TESTITM_CD"
LEFT JOIN "I0940" C
...
LEFT JOIN "I2610" D
  ON B."TESTITM_CD" = D."TESTITM_CD"
LEFT JOIN "I2600" E
  ON D."TESTITM_CD" = E."TESTITM_CD"
WHERE A."TESTITM_CD" IS NOT NULL AND A."TESTITM_CD" != ''
LIMIT 100;
```

## SCN_012 — `HISTTRACEREGNO` 기반 (LOW, 24점)

**참여 데이터셋** (3개): I1930, I1940, I1920

| From | 컬럼명 | To | 컬럼명 | JOIN 유형 | 카디널리티 | 매칭 | A비율 | B비율 | 신뢰도 | 샘플값 |
|---|---|---|---|---|---|---|---|---|---|---|
| 수산물이력정보-생산정보 | 이력추적등록번호(HIST_TRACE_REG_NO) | 수산물이력정보-출하정보 | 이력추적등록번호(HIST_TRACE_REG_NO) | LEFT JOIN (A→B) | N:M | 3 | 60% | 3% | LOW | `1020, 0600` |
| 수산물이력정보-기본정보 | 이력추적등록번호(HIST_TRACE_REG_NO) | 수산물이력정보-생산정보 | 이력추적등록번호(HIST_TRACE_REG_NO) | LEFT JOIN (B→A) | N:M | 3 | 1% | 60% | LOW | `0633, 0600` |
| 수산물이력정보-기본정보 | 이력추적등록번호(HIST_TRACE_REG_NO) | 수산물이력정보-출하정보 | 이력추적등록번호(HIST_TRACE_REG_NO) | LEFT JOIN (B→A) | N:M | 11 | 4% | 12% | LOW | `0633, 1508` |
...
## SCN_CHAIN_008 — `CHAIN:LCNS_NO+RAWMTRLORDNO` 기반 (LOW, 23점) 🔗 브릿지: `C002`

**참여 데이터셋** (3개): C002, I1250, C006

| From | 컬럼명 | To | 컬럼명 | JOIN 유형 | 카디널리티 | 매칭 | A비율 | B비율 | 신뢰도 | 샘플값 |
|---|---|---|---|---|---|---|---|---|---|---|
| 식품(첨가물)품목제조보고(원재료) | 인허가번호(LCNS_NO) | 식품(첨가물)품목제조보고 | 인허가번호(LCNS_NO) | LEFT JOIN (B→A) | N:M | 1 | 0% | 50% | LOW | `19550509001` |
| 식품(첨가물)품목제조보고(원재료) | 원재료표시순서(RAWMTRL_ORDNO) | 축산물품목제조보고(원재료) | 원재료표시순서(RAWMTRL_ORDNO) | LEFT JOIN (B→A) | N:M | 22 | 4% | 38% | LOW | `1,2,3,4,5,6,7,8, 7,8,9,10,11,12` |
...
LEFT JOIN "I1250" B
  ON A."LCNS_NO" = B."LCNS_NO"
LEFT JOIN "C006" C
  ON A."RAWMTRL_ORDNO" = C."RAWMTRL_ORDNO"
WHERE A."LCNS_NO" IS NOT NULL AND A."LCNS_NO" != ''
LIMIT 100;
```

## SCN_009 — `PRDLST_CD` 기반 (LOW, 22점)

**참여 데이터셋** (8개): I0940, I2580, I0950, I0960, I2600, I2610, I2510, I0490

| From | 컬럼명 | To | 컬럼명 | JOIN 유형 | 카디널리티 | 매칭 | A비율 | B비율 | 신뢰도 | 샘플값 |
|---|---|---|---|---|---|---|---|---|---|---|
| 식품용 기구 및 용기.포장 공전 | 품목코드(PRDLST_CD) | 개별기준규격 | 품목분류코드(PRDLST_CD) | LEFT JOIN (A→B) | N:M | 46 | 82% | 25% | MEDIUM | `F0100004000000, F0300000100000` |
| 식품첨가물공전 | 품목코드(PRDLST_CD) | 개별기준규격 | 품목분류코드(PRDLST_CD) | LEFT JOIN (A→B) | N:M | 21 | 88% | 11% | MEDIUM | `D0401300000000, D0300300027000` |
| 건강기능식품공전 | 품목코드(PRDLST_CD) | 개별기준규격 | 품목분류코드(PRDLST_CD) | LEFT JOIN (B→A) | N:M | 112 | 34% | 61% | MEDIUM | `E0301700000000, E0317300000000` |
| 공통기준규격 | 품목분류코드(PRDLST_CD) | 공통기준제외 | 품목코드(PRDLST_CD) | LEFT JOIN (B→A) | N:M | 6 | 4% | 75% | MEDIUM | `C0000000000000, B0203010000000` |
...
| 회수.판매중지 정보 | 품목코드(PRDLST_CD) | 건강기능식품공전 | 품목코드(PRDLST_CD) | LEFT JOIN (A→B) | N:M | 12 | 10% | 4% | LOW | `E0201400000000, E0000000000000` |
...
LEFT JOIN "I0960" D
  ON B."PRDLST_CD" = D."PRDLST_CD"
WHERE A."PRDLST_CD" IS NOT NULL AND A."PRDLST_CD" != ''
LIMIT 100;
```

## SCN_015 — `RAWMTRLORDNO` 기반 (LOW, 21점)
...
## SCN_013 — `BASISLAWORD` 기반 (LOW, 16점)
...
## SCN_014 — `PRDLST_REPORT_NO` 기반 (LOW, 15점)
...
## SCN_CHAIN_003 — `CHAIN:BRCD_NO+PRDLST_CD+RTRVLDSUSESEQ` 기반 (LOW, 15점) 🔗 브릿지: `I0490`

**참여 데이터셋** (4개): I0490, I2620, I0940, I2640
...
SELECT
  ...
  D."ADDR" AS "D_영업자주소",
  D."INSTT_NM" AS "D_검사기관",
  D."REGSTR_TELNO" AS "D_전화번호",
  D."BRCDNO" AS "D_바코드번호",
  D."FRMLCUNIT" AS "D_포장단위",
  D."TEST_ITMNM" AS "D_부적합항목",
  D."STDR_STND" AS "D_기준규격",
  D."TESTANALS_RSLT" AS "D_검사결과",
  D."CRET_DTM" AS "D_등록일",
  D."RTRVLDSUSE_SEQ" AS "D_회수폐기일련번호",
  D."LCNS_NO" AS "D_업체인허가번호",
  D."REPORTR_TELNO" AS "D_보고자전화번호"
FROM "I0490" A
LEFT JOIN "I2620" B
  ON A."BRCDNO" = B."BRCDNO"
LEFT JOIN "I0940" C
  ON A."PRDLST_CD" = C."PRDLST_CD"
LEFT JOIN "I2640" D
  ON A."RTRVLDSUSE_SEQ" = D."RTRVLDSUSE_SEQ"
WHERE A."BRCDNO" IS NOT NULL AND A."BRCDNO" != ''
LIMIT 100;
```

## SCN_CHAIN_009 — `CHAIN:RAWMTRLORDNO+LCNS_NO` 기반 (LOW, 15점) 🔗 브릿지: `C006`

**참여 데이터셋** (3개): C006, C002, I1320
...
## SCN_016 — `DSPSDTLSSEQ` 기반 (LOW, 9점)

**참여 데이터셋** (4개): I0470, I0480, I0481, I0482

| From | 컬럼명 | To | 컬럼명 | JOIN 유형 | 카디널리티 | 매칭 | A비율 | B비율 | 신뢰도 | 샘플값 |
|---|---|---|---|---|---|---|---|---|---|---|
| 행정처분결과 | 행정처분전산키(DSPSDTLS_SEQ) | 행정처분결과(식품제조가공업) | 행정처분전산키(DSPSDTLS_SEQ) | LEFT JOIN (B→A) | 1:1 | 49 | 5% | 19% | LOW | `4605146, 4617344` |
| 행정처분결과 | 행정처분전산키(DSPSDTLS_SEQ) | 행정처분결과(식품판매업) | 행정처분전산키(DSPSDTLS_SEQ) | LEFT JOIN (B→A) | 1:1 | 80 | 8% | 12% | LOW | `4608562, 4612784` |
...
## SCN_018 — `PRSECINSTTRCOGNNO` 기반 (LOW, 9점)
...
## SCN_019 — `AS` 기반 (LOW, 8점)

**참여 데이터셋** (2개): I2390, I2400

| From | 컬럼명 | To | 컬럼명 | JOIN 유형 | 카디널리티 | 매칭 | A비율 | B비율 | 신뢰도 | 샘플값 |
|---|---|---|---|---|---|---|---|---|---|---|
| 토양지하수 토양실태조사정보 | 비소(AS_) | 지하수수질측정망 측정결과 | 비소(AS_) | LEFT JOIN (B→A) | N:M | 2 | 0% | 15% | LOW | `0.0, 0.009` |
...
LEFT JOIN "I2400" B
  ON A."AS_" = B."AS_"
WHERE A."AS_" IS NOT NULL AND A."AS_" != ''
LIMIT 100;
```

## SCN_020 — `PRPOS` 기반 (LOW, 7점)
